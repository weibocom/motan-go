...
func (d *DebugHandler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if req.URL.Path == "/debug/pprof/sw" {
		...
		case "/debug/stat/connections":
			StatConnections(rw)
		case "/debug/endpoint_profile_downgrade":
			ChangeEndpointDowngrade(rw, req)
		default:
			Index(rw, req)
		}
	}
...
func ChangeEndpointDowngrade(w http.ResponseWriter, r *http.Request) {
...
func (h *RuntimeHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/runtime/info":
		result := map[string]interface{}{
			"result":                     "ok",
			motan.RuntimeInstanceTypeKey: "motan-go",
		}
		// cluster info
