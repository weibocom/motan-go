...
func TestEncodeMotanV1Request(t *testing.T) {
	req := &motan.MotanRequest{RequestID: 3452345}
	baseV1Msg := buildV1Msg(t, testV1ReqBytes)
	req.GetRPCContext(true).OriginalMessage = baseV1Msg
...
func TestEncodeMotanV1Response(t *testing.T) {
	res := &motan.MotanResponse{RequestID: 36476555}
	baseV1Msg := buildV1Msg(t, testV1ResBytes)
	res.GetRPCContext(true).OriginalMessage = baseV1Msg
	...
	assertTrue(res2.GetRequestID() == res.RequestID, "v1 res rid", t)
...
func TestParseV1ReqMessage(t *testing.T) {
	tests := []*reqTestInfo{
		{"normal req", &MotanV1Message{testV1ReqBytes, 1, 0, 1743455988312178690, 230}, "com.weibo.motan.demo.service.MotanDemoService", "hello", "java.lang.String", 5},
		{"no param req", &MotanV1Message{testV1NoParamReqBytes, 1, 0, 1743462559279742978, 188}, "com.weibo.motan.demo.service.MotanDemoService", "noParam", "void", 5},
		{"multi params req", &MotanV1Message{testV1MultiParamReqBytes, 1, 0, 1743463219059490817, 338}, "com.weibo.motan.demo.service.MotanDemoService", "rename", "com.weibo.motan.demo.service.model.User,java.lang.String", 5},
		{"compress req", &MotanV1Message{testV1CompressReqBytes, 2, 0, 1743463667605700610, 208}, "v1compressMethodSign", "hell817ff3733269", "", 7},
		{"compress gzip req", &MotanV1Message{testV1CompressGzipReqBytes, 2, 0, 1743463667605700610, 186}, "v1compressMethodSign", "hell817ff3733269", "", 7},
	}
...
func TestParseV1ResMessage(t *testing.T) {
	tests := []*resTestInfo{
		{"normal res", &MotanV1Message{testV1ResBytes, 1, 1, 1743455988312178690, 69}, 1, false, 0},
		{"void res", &MotanV1Message{testV1VoidResBytes, 1, 3, 1743462708025491459, 14}, 1, false, 0},
		{"exception res", &MotanV1Message{testV1ExceptionResBytes, 1, 5, 1743462891537825796, 1960}, 24, true, 0},
		{"compress res", &MotanV1Message{testV1CompressResBytes, 2, 7, 1743466416913252353, 73}, 0, false, 0},
		{"compress gzip res", &MotanV1Message{testV1CompressGzipResBytes, 2, 7, 1743466534460719105, 85}, 0, false, 0},
		{"compress gzip attachment res", &MotanV1Message{testV1CompressWithAttachmentResBytes, 2, 7, 1743466820126375937, 113}, 0, false, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			checkRes(t, tt)
