...
func (c *CommandRegistryWrapper) GetRuntimeInfo() map[string]interface{} {
	info := c.registry.GetRuntimeInfo()
	subscribeInfo := map[string]interface{}{}
	subscribeInfo[motan.RuntimeWeightKey] = c.weights
	if c.staticTcCommand != nil {
		subscribeInfo[motan.RuntimeStaticCommandKey] = c.staticTcCommand
	}
	subscribeInfo[motan.RuntimeAgentCommandKey] = c.agentCommandInfo
	subscribeInfo[motan.RuntimeServiceCommandKey] = c.serviceCommandInfo
	subscribeInfo[motan.RuntimeCommandHistoryKey] = c.commandRecorder.GetRecords()
	subscribeInfo[motan.RuntimeNotifyHistoryKey] = c.notifyRecorder.GetRecords()
	info[motan.RuntimeSubscribeInfoKey] = subscribeInfo
	return info
}
