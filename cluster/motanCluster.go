...
func (m *MotanCluster) GetRuntimeInfo() map[string]interface{} {
	...
	for _, endpoint := range m.Refers {
		...
		if ep.IsAvailable() {
			availableRefers[fmt.Sprintf("%d-%s", i, ep.URL.GetIdentity())] = motan.GetRuntimeInfo(ep.Caller)
		} else {
			unavailableRefers[fmt.Sprintf("%d-%s", i, ep.URL.GetIdentity())] = motan.GetRuntimeInfo(ep.Caller)
		}
		i++
	}
