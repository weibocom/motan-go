...
var (
	...
	accessForbidDispose = flag.Bool("access_forbid_dispose", false, "If true, when access log is in async mode and consume channel is full, incoming access log would not be discarded")
)
...
type defaultLogger struct {
	async                bool
	accessDisposeCounter int64
	accessForbidDispose  bool
...
func (d *defaultLogger) AccessLog(logObject *AccessLogEntity) {
	if d.async {
		if d.accessForbidDispose {
			...
			case d.outputChan <- logObject:
			default:
				atomic.AddInt64(&d.accessDisposeCounter, 1)
			}
		}
	} else {
...
func (d *defaultLogger) SetAsync(value bool) {
	d.async = value
	d.accessForbidDispose = *accessForbidDispose
	if value {
		...
		if !d.accessForbidDispose {
			go func() {
				defer func() {
					if err := recover(); err != nil {
						log.Printf("logs discard accessLog info failed. err:%v", err)
						debug.PrintStack()
					}
				}()
				...
				for {
					select {
					case <-ticker.C:
						if discardCount := atomic.LoadInt64(&d.accessDisposeCounter); discardCount != 0 {
							d.logger.Warn(fmt.Sprintf("accessLog discard, count: %d", discardCount))
							atomic.StoreInt64(&d.accessDisposeCounter, 0)
						}
					}
				}
			}()
