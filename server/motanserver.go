...
func (m *MotanServer) processReq(request *mpro.Message, conn net.Conn) {
	...
	if request.Header.IsHeartbeat() {
		...
		if err != nil {
			...
			res = mpro.BuildExceptionResponse(request.Header.RequestID, mpro.ExceptionToJSON(&motan.Exception{ErrCode: 500, ErrMsg: "deserialize fail. " + "err:not found serialization name." + " method:" + request.Metadata[mpro.MMethod], ErrType: motan.ServiceException}))
		} else {
